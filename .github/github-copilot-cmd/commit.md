...
1. **Scope OBLIGATOIRE** — Jamais de commit sans scope : `fix(auth):` pas `fix:`
2. **Message court** — Max 72 caractères, idéalement 50
3. **Présent** — "add feature" pas "added feature"
4. **Lowercase** — Après le `:`, tout en minuscule
5. **Pas de point final** — Économie d'espace
6. **Un commit = un changement logique** — Si plusieurs changes distincts, suggérer de splitter
7. **CHANGENOTE obligatoire** — Toujours générer sauf si `--no-note`
