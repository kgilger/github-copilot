...
> ⚠️ **CRITIQUE** : Tu DOIS exécuter TOUTES les étapes du workflow, notamment :
> 1. `git add -A` + `git commit`
> 2. **CRÉER LE FICHIER CHANGENOTE** avec `create_file` — NE JAMAIS OUBLIER
> 
> Si tu termines sans avoir créé le fichier `CHANGENOTES/*.md`, tu as ÉCHOUÉ.
...
```bash
git add -A                           # Si pas déjà stagé
git commit -m "type(scope): message"
```

### 7. Générer la CHANGENOTE

Après le commit, créer un fichier dans `CHANGENOTES/` **à la racine du projet** (PAS dans `.github/`) :

**Emplacement** : `<project-root>/CHANGENOTES/`
...
> ⚠️ **RÈGLE #1 ABSOLUE** : Après le `git commit`, tu DOIS créer le fichier CHANGENOTE. Pas d'exception (sauf `--no-note`).

1. **CHANGENOTE OBLIGATOIRE** — Après chaque commit, créer `CHANGENOTES/<date>_<type>_<scope>.md` avec `create_file`
2. **Scope OBLIGATOIRE** — Jamais de commit sans scope : `fix(auth):` pas `fix:`
3. **Message court** — Max 72 caractères, idéalement 50
4. **Présent** — "add feature" pas "added feature"
5. **Lowercase** — Après le `:`, tout en minuscule
6. **Pas de point final** — Économie d'espace
7. **Un commit = un changement logique** — Si plusieurs changes distincts, suggérer de splitter
8. **NO verbose** — Pas d'explication longue, juste le commit
9. **NO interactive** — Pas de demande de confirmation
10. **NO signature** — Pas de "Generated by Copilot"
