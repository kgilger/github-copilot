# Best Practices — Catalogue par Domaine

Ce fichier contient les bonnes pratiques à injecter dans les commandes générées par `/create-cmd`.
...
Règles applicables à **TOUTES** les commandes :

### Exécution
- **Pas d'interactif** — Aucune commande demandant une saisie utilisateur
- **Exit silencieux** — Si rien à faire, terminer sans message ni erreur
- **Pas de verbose** — Messages concis, uniquement l'essentiel
- **Pas de signatures** — Jamais de "Generated by...", "Created with..." ou équivalent

### Validation
- **Prérequis AVANT exécution** — Toujours vérifier les conditions avant de commencer
- **Fail fast** — Arrêter dès qu'un prérequis n'est pas rempli
- **Messages d'erreur clairs** — Reporter les erreurs sans verbosité excessive

### Structure
- **Sections obligatoires** — Description, Allowed Tools, Validation, Workflow, Rules
- **Workflow numéroté** — Étapes claires et ordonnées
- **Arguments documentés** — Expliquer comment `$ARGUMENTS` est utilisé
